module UsersHelper
	...
	def get_users id, position
		sql = "WITH RECURSIVE child_users(id, sponsered_by_id,h_parent,position,lvl, sponser_id) AS " +
			"( SELECT id, sponsered_by_id, h_parent, position, sponser_id, 0 FROM users " +
			"WHERE users.id = #{id} " +
			"UNION ALL SELECT  users.id, users.sponsered_by_id, users.h_parent,users.position, users.sponser_id ,child_users.lvl+1 FROM users JOIN child_users " +
			"ON child_users.id = users.h_parent and users.position = '#{position}') " +
			"SELECT id, sponsered_by_id, h_parent,lvl, sponser_id FROM child_users where position ='#{position}'; "
		records_array = ActiveRecord::Base.connection.execute(sql).to_a
		if records_array.present?
